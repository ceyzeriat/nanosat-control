...
class CCSDSCategory(object):
    def __init__(self, name, number, aux_trousseau=None, verbose='',
        ...
        if name != fcts.clean_name(name, allow_space=True):
            raise exc.WrongNameFormat(name)
        self.name = str(name)
        self.verbose = str(verbose)
        self._thatsTCANS = bool(thatsTCANS)
        self.table_name = fcts.clean_name(self.name.replace(' ', '_')).lower()
        inf = inflect.engine()
        ...
        self.object_name = fcts.camelize_singular(self.table_name)
        if self.object_name is False:
            raise exc.WrongCategoryObjectName(self.object_name)
        if self.table_name != fcts.camelize_singular_rev(self.object_name):
            raise exc.WrongCategoryTableName(self.table_name)
        ...
        if self.data_file is None:
            ...
            self.data_trousseau = getattr(categories,
                                          self.data_file).TROUSSEAU
