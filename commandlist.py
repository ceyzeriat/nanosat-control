...
class CommandList(object):
    ...
    def loadCMDS(self):
        ...
        will be lost
        """
        self.allcmds = core.load_json_cmds(param_commands.COMMANDSFILE)
    ...
    def apply(self):
        ...
        """
        core.save_json_cmds(param_commands.COMMANDSFILE, cmds=self.allcmds)

    def loadCSV(self, filename, titles_row=0, delimiter='#'):
        """
        Loads the csv import-file
        """
        csvcontent = csv.reader(open(str(filename)), delimiter=str(delimiter)[0])
        # skip the shit
        ...
        for line in itera:
            ...
            if line[param_commands.CSVSUBSYSTEM] != "":
                ...
                cm = {'number': int(line[param_commands.CSVNUMBER]),
                        'name': str(line[param_commands.CSVNAME])\
                                            .strip().replace(' ', '_'),
        ...
        for item in self.csvcontent:
            if item['number'] in ids_to_add:
                if item['number'] in allids\
                    ...
                    to_add.append(ctrl.cmd.cm.Cm(**item))
                    cnt += 1
        self.csvcontent += to_add
        print("Added '{:d}' commands".format(cnt))
