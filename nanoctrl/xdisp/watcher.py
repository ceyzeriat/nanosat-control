...
from nanoparam.param_apid import PIDREGISTRATION_REV
...
class Xdisp(object):
    ...
    def set_time(self):
        ...
        if not self.running:
            return
        self.disp(self.bar,
                   PrintOut(fcts.now().strftime('%T'), opts=self.CYAN))
        """ts = fcts.now().totimestamp()
    ...
    def report(self, message):
        ...
        if not self.running:
            return
        self.disp(self.RP,
                  PrintOut('{} {}'.format(fcts.now().strftime('%T'), message),
    ...
    def add_TC(self, packet_id, infos):
        ...
        TC = PrintOut(TCFMT.format(timestamp=fcts.now().strftime(TIMESTAMPFMT),
    ...
    def add_TM(self, packet_id, infos):
        ...
        pidstr = str(PIDREGISTRATION_REV[pid][pld][lvl])\
                        if pid in PIDREGISTRATION_REV.keys() else "?????"
        TM = PrintOut(TMFMT.format(timestamp=fcts.now().strftime(TIMESTAMPFMT),
                                   pld=PAYLOADICO if pld == 1 else OBCICO,
                                   lvl=L1ICO if lvl == 1 else L0ICO,
                                   pid=pidstr,
                                   pkid=str(packet_id),
